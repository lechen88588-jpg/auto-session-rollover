...
大多数 API 提供商对超过 200k 的输入收取额外费用：

| 模型 | ≤200k 价格 | >200k 价格 |
|------|-----------|-----------|
| MiMo v2 Pro | $1/M input | 超 256k 翻倍 → $2/M |
| Claude Opus 4.6 | 标准价 | 标准价（无额外费用） |
| GPT-5.4 | 标准价 | 超 272k 输入翻倍 + 输出 1.5x |

**建议：** 日常使用 `200` 即可，只有处理超长文档/代码库时才临时调大。
